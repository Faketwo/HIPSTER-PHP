...
//判斷是否選擇商品，和商品數量
if($_POST["productId"] == 0 || (int)$_POST["amountId"] <=0){
...
//判斷是不是選取一樣的商品
$sqlmatch = "SELECT * FROM `item_lists` 
            WHERE `itemListId` = {$_POST['itemListId']}
            AND `productId` = {$_POST['productId']}";
$stmtmatch = $pdo->prepare($sqlmatch);
$stmtmatch->execute();
if($stmtmatch->rowCount() > 0 ){
    $arrmatch = $stmtmatch->fetchAll(PDO::FETCH_ASSOC)[0];
    //確定商品數量是否足夠
    ...
    $total = $arrmatch['checkPrice'] * (int)$_POST['amountId'];
    $sqlItemList = "UPDATE `item_lists` SET `checkQty`= ?,`checkSubtotal` =?
    WHERE `itemListId`= ?";
    ...
    if($stmtItemList->rowCount()>0){
        ...
        $arrParamamount = [
            ($arrprice['productAmount'] + $arrmatch['checkQty']) - $_POST["amountId"],
            $_POST["productId"]
        ];
