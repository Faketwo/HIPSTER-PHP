...
for($i = 0; $i < count($_FILES["multipleImageImg"]["name"]); $i++){
    ...
    if( $_FILES["multipleImageImg"]["error"][$i] === 0 ) {
        ...
        if( !move_uploaded_file($_FILES["multipleImageImg"]["tmp_name"][$i], "../images/article_column/multiple_images/{$multipleImageImg}") ) {
    ...
    //SQL 敘述
    $sql = "INSERT INTO `article_multiple_images` (`multipleImageImg`,`articleId`) VALUES (?, ?)";
